...
capture = cv2.VideoCapture(filename="./bridges/dataset/한남대교/2023-10-12 11-57-07oneway.mov")
object_detector = cv2.createBackgroundSubtractorMOG2(history=1000, varThreshold=32, detectShadows=False)
...
while True:
    ...
    frame_Region_of_Interest = frame[500:670, 1620:1720]
    ...
    for box_id in boxes_ids:
        x, y, w, h, id = box_id
        cv2.putText(img=frame_Region_of_Interest, text=str(id), org=(x, y), fontFace=cv2.FONT_HERSHEY_PLAIN, fontScale=2, color=(0, 0, 255), thickness=3)
        cv2.rectangle(img=frame_Region_of_Interest, pt1=(x, y), pt2=(x+w, y+h), color=(0, 0, 255), thickness=3)
    ...
    cv2.imshow(winname="HanNam Bridge", mat=frame)
...
"""make the file"""
with open(file="bridges/measured_list/한남.txt", mode='w') as file:
